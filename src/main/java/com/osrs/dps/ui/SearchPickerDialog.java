...
/** A generic type-to-filter picker dialog with a search box and a result list. */
public final class SearchPickerDialog<T> {
    ...
    public T showAndPick() {
        ...
        listView.setPrefSize(420, 380);
        listView.setCellFactory(lv -> new javafx.scene.control.ListCell<>() {
            @Override
            protected void updateItem(T item, boolean empty) {
                super.updateItem(item, empty);
                setText(empty || item == null ? null : labeller.apply(item));
            }
        });
